...
def _build_main_afl_clauses(user, search, customer, task_report, executor_org,
    ...
    if user.effective_role in ("оператор", "работник"):
        clauses.append(f"executor IN (SELECT executor_name FROM users WHERE locale = :locale)")
...
async def api_main_afl_stats(request: Request, db_session: AsyncSession) -> Response:
    ...
    if user.effective_role in ("оператор", "работник"):
        base_where += f" AND executor IN (SELECT executor_name FROM users WHERE locale = :locale)"
        params["locale"] = user.locale
    elif user.effective_role == "менеджер":
    ...
    ex_result = await db_session.execute(
        text(f"""SELECT m.executor, m.cnt, u.locale
                 ...
                 LEFT JOIN users u ON u.executor_name = m.executor
