...
async def api_executors(request: Request, db_session: AsyncSession) -> Response:
    ...
    if user.effective_role in ("оператор", "работник"):
        result = await db_session.execute(text(
            f"SELECT DISTINCT executor FROM main_afl WHERE executor IN (SELECT executor_name FROM users WHERE locale = :locale) AND executor IS NOT NULL ORDER BY executor"),
            {"locale": user.locale})
...
async def api_task_reports(request: Request, db_session: AsyncSession) -> Response:
    ...
    params = {}
    if user.effective_role in ("оператор", "работник"):
        query += f" AND executor IN (SELECT executor_name FROM users WHERE locale = :locale)"
