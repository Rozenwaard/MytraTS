...
- **Палитра (тёплая, light)**: фон `#F7F6F4`, поверхности/сайдбар белые `#FFFFFF`, primary `#1E5B45`, шапка `#ffeccc`, лого `#fea201`; акценты `#87643e` (реестр), `#ff6e4e` (удаление/ошибки), `#ffe02a` (резерв). Тёмную тему пока не трогаем.
- Логотип «три молнии» (Mitra) — `logo.svelte`, path `M13 2L3 14h9l-1 8 10-12h-9l1-8z`, бейдж `bg-brand-logo`.
- Шелл: сайдбар слева (сворачивается в рельсу `w-56 ↔ w-16`) + шапка: слева поиск (стор `search.svelte.ts`, дебаунс 300 мс → параметр `search`), справа ФИО/тема/выход.
...
- Индикаторы строк: Реестр `Р` (жирная `#87643e`; «Отклонён» → `#ffeccc`), Ошибки `О` (жирная `#ff6e4e`), Норматив — число по центру; пустые значения — прочерк `-`.
...
- ✅ auth (логин/логаут/гвард), шелл (сворачиваемый сайдбар), тема, поиск в шапке, «Обзор» (таблица: 10 колонок + раскрытие группами, липкая шапка, sort/pagination), действия в раскрытии («Удалить из реестра», «Заменить вид работ»), тосты.
- ⬜ Фильтры/статистика над таблицей; Загрузка/Список/Настройка; «В реестр»/«Выбрать всё»; динамика колонок из `/api/user/settings`; Дашборд/Отчёты/Помощь.
