...
- Действия в раскрытии: «Удалить из реестра» (destructive, `POST /api/reestr/reset`), «Печать реестра» (менеджер/оператор/работник, `GET /api/download-reestr/{rn}`) и «Заменить вид работ» (select из `/api/task-reports` + кнопка, только админ; «Не выполнено» = NULL, сбрасывает reestr_number/reestr_date) + тосты (`store/toast.svelte.ts`).
...
└── routes/
    ...
    └── (app)/                        # защищённая зона
        ...
        ├── main-afl/list             # «Список» (плашки/поиск/печать/пустой/удаление + таблица)
...
- ✅ auth (логин/логаут/гвард), шелл (топбар без сайдбара + бар табов нижнего уровня), тема, «Обзор» (таблица: 10 колонок + раскрытие группами, липкая шапка, sort/pagination, чекбокс-выбор + «В реестр»), фильтры (статистика/виды работ/исполнители-отделения/дата/свич «Все строки-Без реестра» + сброс), действия в раскрытии («Удалить из реестра», «Печать реестра», «Заменить вид работ»), «Загрузка» (`.xlsx` → raw_afl, прогресс + опрос `/api/upload/progress`), «Список» (плашки/поиск/печать/пустой/удаление + таблица), ПКМ-копирование № задания, тосты.
- ⬜ Настройка (порядок/видимость колонок); динамика колонок из `/api/user/settings`; Дашборд/Отчёты/Помощь.
- ⬜ Топ-меню «Админ» (только администратор): страница `/admin` с полным CRUD `users` (поле `executor_name` + список сирот `main_afl.executor`); бэк `/api/admin/*`.
