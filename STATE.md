...
- `main_afl.errors` — колонка с найденными ошибками (через `; `). Пересчитывается при загрузке (`recompute_errors`, только для `verified='Нет'` И `sent_to_billing='Нет'` И `status='Завершено'`) и миграцией `_migrate_errors.py`.
