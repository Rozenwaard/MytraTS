...
- `main_afl.errors` — колонка с найденными ошибками (через `; `). При загрузке все ошибки очищаются, затем пересчитываются только для затронутых строк с `verified='Нет'` И `sent_to_billing='Нет'` И `status='Завершено'`; полный пересчёт — миграцией `_migrate_errors.py`.
