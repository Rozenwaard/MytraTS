...
- `main_afl.errors` — колонка с найденными ошибками (через `; `). При загрузке очищаются только затронутые строки (слияние: вставленные/обновлённые), затем для них пересчитываются ошибки (при `verified='Нет'` И `sent_to_billing='Нет'` И `status='Завершено'`); старые строки не трогаются. Полный пересчёт — миграцией `_migrate_errors.py`.
